...
/* 	Copyright (C) 2012 - 2013  Red Pixel Studios  (email : support@redpixel.com)
...
class RPS_Include_Content {
	...
	public function cb_include_shortcode( $atts, $content = null ) {
		...
		$the_content .= ( $filter ) ? apply_filters( 'the_content', $the_article) : do_shortcode( $the_article );
	...
	public function _admin_menu() {
		add_options_page( sprintf( __( '%1$s Settings', 'rps-include-content' ), self::PLUGIN_NAME ), 'Include Content', 'manage_options', self::PLUGIN_PREFIX . 'options_page', array( $this, '_admin_page_options' ) );
	}
						...
						<td>
							<fieldset>
								...
								<label for="<?php echo esc_attr( self::PLUGIN_PREFIX . 'disable_hover' ); ?>">
									<input type="checkbox" id="<?php echo esc_attr( self::PLUGIN_PREFIX . 'disable_hover' ); ?>" name="<?php echo esc_attr( self::PLUGIN_PREFIX . 'disable_hover' ); ?>" value="hover" <?php echo ((esc_attr(( ( is_multisite() ) ? get_blog_option( $current_blog_id, '_' . self::PLUGIN_PREFIX . 'disable_hover' ) : get_option( '_' . self::PLUGIN_PREFIX . 'disable_hover' ) )) ) ? "checked" : "") ?> /> <?php _e( 'Remove the interface that appears when hovering over included content', 'rps-include-content' ); ?>
								</label>
								<br>
								<label for="<?php echo esc_attr( self::PLUGIN_PREFIX . 'private_content' ); ?>">
									<input type="checkbox" id="<?php echo esc_attr( self::PLUGIN_PREFIX . 'private_content' ); ?>" name="<?php echo esc_attr( self::PLUGIN_PREFIX . 'private_content' ); ?>" value="private" <?php echo ((esc_attr(( ( is_multisite() ) ? get_blog_option( $current_blog_id, '_' . self::PLUGIN_PREFIX . 'private_content' ) : get_option( '_' . self::PLUGIN_PREFIX . 'private_content' ) )) ) ? "checked" : "") ?> /> <?php _e( 'Show private included content without restriction', 'rps-include-content' ); ?>
								</label>
							</fieldset>
						</td>
					</tr>
	...
	public function _admin_action_save_settings() {
		...
		if(isset($fields[ self::PLUGIN_PREFIX . 'disable_hover' ]))
			...
			if ( is_multisite() )
				update_blog_option( $current_blog_id, '_' . self::PLUGIN_PREFIX . 'disable_hover', false );
			else
		...
		if(isset($fields[ self::PLUGIN_PREFIX . 'private_content' ]))
			...
			if ( is_multisite() )
				update_blog_option( $current_blog_id, '_' . self::PLUGIN_PREFIX . 'private_content', false );
			else
